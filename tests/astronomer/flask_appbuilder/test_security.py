...
class TestAstroSecurityManagerMixin:
    ...
    def test_signed_jwt_valid_claims_new_user(
        self, appbuilder, signed_jwt, valid_claims
    ):
    ...
    def test_signed_jwt_valid_claims_existing_user(
        self, appbuilder, user, signed_jwt, valid_claims, mocker
    ):
        ...
        assert resp.status_code == 200
        assert g.user.is_anonymous is False
        appbuilder.sm.manage_user_roles.assert_called_with(
            mocker.ANY, valid_claims["roles"]
        )

        appbuilder.session.refresh(g.user)
        assert g.user.first_name == "John McAirflower"

    def test_signed_jwt_misaligned_roles(
        self, appbuilder, user, signed_jwt, valid_claims, mocker
    ):
    ...
    def test_manage_user_roles__manage_subset(
        self, appbuilder, user, role, monkeypatch
    ):
        """
    ...
    def test_not_yet_valid(
        self, appbuilder, signed_jwt, valid_claims, monkeypatch, leeway
    ):
        monkeypatch.setattr(appbuilder.sm, "validity_leeway", leeway)
...
class TestAirflowAstroSecurityManger:
    ...
    def test_leeway(self, appbuilder, monkeypatch, leeway):
        monkeypatch.setitem(
            os.environ, "AIRFLOW__ASTRONOMER__JWT_VALIDITY_LEEWAY", str(leeway)
        )
        sm = AirflowAstroSecurityManager(appbuilder)
